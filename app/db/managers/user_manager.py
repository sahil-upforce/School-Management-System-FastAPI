...
class UserManager(BaseManager):
    ...
    def create_permission_as_per_users(user_type, user_id, db):
        if user_type == "student":
            ...
            PermissionManager.create_super_user_permissions(
                super_user_id=user_id, db=db
            )
    ...
    def create(model, data, db: Session):
        ...
        UserManager.create_permission_as_per_users(
            user_type=user.user_type, user_id=user.id, db=db
        )
        return user
    ...
    def get_by_email(model, email, db: Session):
        return (
            db.query(model)
            .filter(model.email == email, model.is_active == True)
            .first()
        )
    ...
    def get_by_username(model, username, db: Session):
        return (
            db.query(model)
            .filter(model.username == username, model.is_active == True)
            .first()
        )
    ...
    def get_by_phone(model, phone, db: Session):
        return (
            db.query(model)
            .filter(model.phone == phone, model.is_active == True)
            .first()
        )

    @staticmethod
    def check_conflicts(data: dict, db: Session):
        UserManager.check_username_is_already_exists(
            username=data.get("username"), db=db
        )
