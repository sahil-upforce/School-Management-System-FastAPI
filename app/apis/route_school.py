...
@school_router.post(
    path="/", dependencies=[Depends(jwt_bearer)], response_model=SchoolSchema
)
def create_school(
    school: SchoolCreateSchema,
    db: Session = Depends(get_db),
    authorized: bool = Depends(
        PermissionChecker(required_permissions=["school: create"])
    ),
):
...
@school_router.get(
    path="/", dependencies=[Depends(jwt_bearer)], response_model=List[SchoolSchema]
)
def fetch_all_schools(
    db: Session = Depends(get_db),
    authorized: bool = Depends(
        PermissionChecker(required_permissions=["school: read"])
    ),
):
...
@school_router.get(
    path="/{school_id}", dependencies=[Depends(jwt_bearer)], response_model=SchoolSchema
)
...
def update_school(
    ...
    db: Session = Depends(get_db),
    authorized: bool = Depends(
        PermissionChecker(required_permissions=["school: change"])
    ),
):
    school = SchoolService.update_school(
        school_id=school_id, school_schema=school, db=db
    )
    if not school:
        raise HTTPException(status_code=404, detail="School not found")
    return school
...
def delete_school(
    ...
    db: Session = Depends(get_db),
    authorized: bool = Depends(
        PermissionChecker(required_permissions=["school: delete"])
    ),
):
    ...
    if not school:
        raise HTTPException(status_code=404, detail="School not found")
    return JSONResponse(
        content={"detail": f"School {school.id} is deleted"}, status_code=200
    )
